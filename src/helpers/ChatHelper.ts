...
import { ConfigHelper, EnvironmentHelper, ApiHelper } from '.';
...
export class ChatHelper {
    ...
    static joinRoom(roomName: string) {
        if (ChatHelper.socketConnected) {
            ChatHelper.socket.send(JSON.stringify({ 'action': 'joinRoom', 'room': roomName, 'token': ApiHelper.getConfig("StreamingLiveApi")?.jwt || "" }));
        }
    }
    ...
    static sendMessage(room: string, content: string) {
        var c = content.trim();
        if (c !== '') {
            ChatHelper.socket.send(JSON.stringify({ 'action': 'sendMessage', 'room': room, 'userGuid': ChatHelper.user.guid, 'name': ChatHelper.user.displayName, 'msg': c, 'token': ApiHelper.getConfig("StreamingLiveApi").jwt }));
        }
    }
    ...
    static sendFacebook(room: string, content: string, name: string) {
        var c = content.trim();
        if (c !== '') {
            ChatHelper.socket.send(JSON.stringify({ 'action': 'sendMessage', 'room': room, 'userGuid': ChatHelper.user.guid, 'name': name, 'msg': c, 'token': ApiHelper.getConfig("StreamingLiveApi").jwt }));
        }
    }

    static sendDelete(room: string, ts: number) {
        ChatHelper.socket.send(JSON.stringify({ 'action': 'deleteMessage', 'room': room, 'ts': ts, 'token': ApiHelper.getConfig("StreamingLiveApi").jwt }));
    }

    static setCallout(room: string, content: string) {
        ChatHelper.socket.send(JSON.stringify({ 'action': 'setCallout', 'room': room, 'msg': content, 'token': ApiHelper.getConfig("StreamingLiveApi").jwt }));
    }
