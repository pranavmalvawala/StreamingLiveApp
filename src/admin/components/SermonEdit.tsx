...
import { SermonInterface, ApiHelper, InputBox } from ".";
import { Loading } from "../../appBase/components";
import { DateHelper, UniqueIdHelper } from "../../helpers";
import { Duration, PlaylistInterface } from "../settings/components";
...
export const SermonEdit: React.FC<Props> = (props) => {
  ...
  const loadData = () => {
    ApiHelper.get("/playlists", "StreamingLiveApi").then(data => {
      setPlaylists(data);
      setIsLoading(false);
    });
  }
  ...
  const handleDelete = () => {
    if (window.confirm("Are you sure you wish to delete this sermon?")) {
      ApiHelper.delete("/sermons/" + currentSermon.id, "StreamingLiveApi").then(() => { setCurrentSermon(null); props.updatedFunction(); });
    }
  }
  ...
  const handleSave = () => {
    setSermonUrl();
    ApiHelper.post("/sermons", [currentSermon], "StreamingLiveApi").then(props.updatedFunction);
  }
  ...
  const fetchYoutube = () => {
    ApiHelper.getAnonymous("/sermons/lookup?videoType=youtube&videoData=" + currentSermon.videoData, "StreamingLiveApi").then(d => {
      let v = { ...currentSermon };
      v.title = d.title;
