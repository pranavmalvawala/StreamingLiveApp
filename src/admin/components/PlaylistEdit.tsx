...
import { PlaylistInterface, ApiHelper, InputBox, DateHelper } from ".";
...
export const PlaylistEdit: React.FC<Props> = (props) => {
  ...
  const handleDelete = () => {
    if (window.confirm("Are you sure you wish to delete this playlist?")) {
      ApiHelper.delete("/playlists/" + currentPlaylist.id, "StreamingLiveApi").then(() => { setCurrentPlaylist(null); props.updatedFunction(); });
    }
  }
  ...
  const handleSave = () => {
    ApiHelper.post("/playlists", [currentPlaylist], "StreamingLiveApi").then(props.updatedFunction);
  }
